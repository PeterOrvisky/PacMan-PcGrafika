...
func _ready():
	var pellets = self.get_children() as Array[PelletLocal]
	...
	for ghost in ghost_array:
		if !(ghost is Ghost):
			push_error("Object in ghost_array is not of type Ghost!")
			continue
		
		if !ghost.has_signal("run_away_timeout"):
			push_error("Ghost does not have a signal 'run_away_timeout'!")
			continue
		
		ghost.run_away_timeout.connect(on_ghost_run_away_timeout)

func on_pellet_eaten(should_allow_eating_ghosts: bool, score_value: int):
	# Pridanie skóre za pellet
	points_manager.add_points(score_value)
